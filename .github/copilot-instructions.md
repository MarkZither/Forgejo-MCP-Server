...
All code must conform to the K&R (Kernighan & Ritchie) style as defined in `.editorconfig`.

**Enforcement:**
- Opening braces on the same line (not next line)
- Private fields prefixed with underscore (`_fieldName`)
- 4 spaces for indentation (no tabs)
- `dotnet format` is run automatically on pre-commit via Husky.NET

**Pre-commit Hook:**
- The `.husky/pre-commit` PowerShell script runs `dotnet format --verify-no-changes`
- Commits are blocked if formatting violations are detected
